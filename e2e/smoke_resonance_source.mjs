...
(async () => {
  ...
  await context.route('**/puzzles/levels.json', async (route) => {
    ...
    levels.push({
      ...
      colors: ['red', 'cyan', 'amber', 'green', 'violet', 'white'],
      capacity: 6,
      clearCharges: 0,
      targetMoves: 20,
      towers: [
        ...
        { bands: [{ color: 'red', amplified: false, noisy: false, locked: false }, { color: 'red', amplified: false, noisy: false, locked: false }, { color: 'cyan', amplified: false, noisy: false, locked: false }], dampened: false },
        { bands: [], dampened: false },
        { bands: [{ color: 'amber', amplified: false, noisy: false, locked: true }, { color: 'red', amplified: false, noisy: false, locked: false }, { color: 'red', amplified: false, noisy: false, locked: true }], dampened: false },
        { bands: [{ color: 'red', amplified: false, noisy: false, locked: false }], dampened: false },
        { bands: [], dampened: false }
      ]
    });
  ...
  // Scenario A: move cyan off two reds (tower 0 -> empty 1). Source-side resonance.
  await clickTower(0);
  await clickTower(1);
  // Scenario B: move cyan off two reds (tower 2 -> empty 3).
  await clickTower(2);
  await clickTower(3);
  // Scenario C: place red on tower [amberL, red, redL] to unlock/compress.
  await clickTower(5);
  await clickTower(4);
  ...
  await context.close();
  await browser.close();
  console.log('RESONANCE_SOURCE_OK');
})();
